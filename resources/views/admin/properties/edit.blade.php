<x-admin-layout>
    <x-slot name="title">Edit Flat Listing</x-slot>

    <!-- Header Section -->
    <div class="md:flex md:items-center md:justify-between mb-8">
        <div class="flex-1 min-w-0">
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">
                Edit Flat Listing: {{ $property->property_code }}
            </h1>
            <p class="mt-2 text-sm text-slate-500">
                Modify details of the flat listing. Make sure to keep the private owner records accurate.
            </p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.properties.update', $property) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        ...
        <div class="bg-white border border-slate-200/80 rounded-2xl shadow-sm p-6 lg:p-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="title" id="title" required value="{{ old('title', $property->title) }}" placeholder="e.g. Cozy 1 BHK near Batla House Metro" class="block w-full rounded-xl border-slate-200/85 focus:border-indigo-500 focus:ring-indigo-500 text-sm py-3">
                </div>

                <!-- Description -->
                <div class="md:col-span-2">
                    <label for="description" class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Description</label>
                    <textarea name="description" id="description" rows="4" placeholder="Highlight key features, spacing, layout..." class="block w-full rounded-xl border-slate-200/85 focus:border-indigo-500 focus:ring-indigo-500 text-sm py-3">{{ old('description', $property->description) }}</textarea>
                </div>

                <!-- Rent -->
                <div>
                    <label for="rent" class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Monthly Rent (INR)</label>
                    <input type="number" name="rent" id="rent" required value="{{ old('rent', $property->rent) }}" placeholder="e.g. 15000" class="block w-full rounded-xl border-slate-200/85 focus:border-indigo-500 focus:ring-indigo-500 text-sm py-3">
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="deposit" id="deposit" required value="{{ old('deposit', $property->deposit) }}" placeholder="e.g. 30000" class="block w-full rounded-xl border-slate-200/85 focus:border-indigo-500 focus:ring-indigo-500 text-sm py-3">
                </div>
                ...
                <div>
                    ...
                    <select name="property_type" id="property_type" required class="block w-full rounded-xl border-slate-200/85 focus:border-indigo-500 focus:ring-indigo-500 text-sm py-3">
                        ...
                        <option value="pg" {{ old('property_type', $property->property_type) === 'pg' ? 'selected' : '' }}>PG / Hostel Room</option>
                        <option value="room" {{ old('property_type', $property->property_type) === 'room' ? 'selected' : '' }}>Single Room</option>
                        <option value="house" {{ old('property_type', $property->property_type) === 'house' ? 'selected' : '' }}>Independent House</option>
                        <option value="studio" {{ old('property_type', $property->property_type) === 'studio' ? 'selected' : '' }}>Studio Apartment</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="bhk" id="bhk" required value="{{ old('bhk', $property->bhk) }}" min="1" max="10" class="block w-full rounded-xl border-slate-200/85 focus:border-indigo-500 focus:ring-indigo-500 text-sm py-3">
                </div>

                <!-- Area -->
                <div>
                    <label for="area" class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Super Area (Sq Ft)</label>
                    <input type="number" name="area" id="area" required value="{{ old('area', $property->area) }}" placeholder="e.g. 950" class="block w-full rounded-xl border-slate-200/85 focus:border-indigo-500 focus:ring-indigo-500 text-sm py-3">
                </div>

                <!-- Floor -->
                <div>
                    <label for="floor" class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Floor</label>
                    <input type="text" name="floor" id="floor" required value="{{ old('floor', $property->floor) }}" placeholder="e.g. 2nd Floor, G-1, Ground" class="block w-full rounded-xl border-slate-200/85 focus:border-indigo-500 focus:ring-indigo-500 text-sm py-3">
                </div>
                ...
                <div>
                    ...
                    <select name="furnishing" id="furnishing" required class="block w-full rounded-xl border-slate-200/85 focus:border-indigo-500 focus:ring-indigo-500 text-sm py-3">
                        ...
                        <option value="furnished" {{ old('furnishing', $property->furnishing) === 'furnished' ? 'selected' : '' }}>Fully Furnished</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="availability" id="availability" required class="block w-full rounded-xl border-slate-200/85 focus:border-indigo-500 focus:ring-indigo-500 text-sm py-3">
                        <option value="immediate" {{ old('availability', $property->availability) === 'immediate' ? 'selected' : '' }}>Immediate / Ready to Move</option>
                        <option value="specific_date" {{ old('availability', $property->availability) === 'specific_date' ? 'selected' : '' }}>Specific Date</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="locality_id" id="locality_id" required class="block w-full rounded-xl border-slate-200/85 focus:border-indigo-500 focus:ring-indigo-500 text-sm py-3">
                        @foreach($localities as $locality)
                            <option value="{{ $locality->id }}" {{ old('locality_id', $property->locality_id) == $locality->id ? 'selected' : '' }}>{{ $locality->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="approximate_location" id="approximate_location" required value="{{ old('approximate_location', $property->approximate_location) }}" placeholder="e.g. Near Batla House Metro, Jamia Nagar" class="block w-full rounded-xl border-slate-200/85 focus:border-indigo-500 focus:ring-indigo-500 text-sm py-3">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="nearest_metro" id="nearest_metro" value="{{ old('nearest_metro', $property->nearest_metro) }}" placeholder="e.g. Okhla Vihar" class="block w-full rounded-xl border-slate-200/85 focus:border-indigo-500 focus:ring-indigo-500 text-sm py-3">
                </div>

                <!-- Landmark -->
                <div>
                    <label for="landmark" class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Landmark</label>
                    <input type="text" name="landmark" id="landmark" value="{{ old('landmark', $property->landmark) }}" placeholder="e.g. Behind Khalilullah Mosque" class="block w-full rounded-xl border-slate-200/85 focus:border-indigo-500 focus:ring-indigo-500 text-sm py-3">
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                ...
                @foreach($amenities as $amenity)
                    <div class="flex items-center">
                        <input id="amenity_{{ $amenity->id }}" name="amenities[]" type="checkbox" value="{{ $amenity->id }}" class="h-4.5 w-4.5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" {{ is_array(old('amenities', $propertyAmenityIds)) && in_array($amenity->id, old('amenities', $propertyAmenityIds)) ? 'checked' : '' }}>
        ...
        <!-- Section 3: Private / Owner Information -->
        <div class="bg-indigo-950 text-white rounded-2xl shadow-md p-6 lg:p-8 space-y-6 border border-indigo-900">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="owner_name" id="owner_name" required value="{{ old('owner_name', $property->owner_name) }}" placeholder="e.g. Mohammad Asif" class="block w-full rounded-xl bg-indigo-900/40 border-indigo-800 text-white placeholder-indigo-400 focus:border-indigo-400 focus:ring-indigo-400 text-sm py-3">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="owner_contact" id="owner_contact" required value="{{ old('owner_contact', $property->owner_contact) }}" placeholder="e.g. +91 99999 88888" class="block w-full rounded-xl bg-indigo-900/40 border-indigo-800 text-white placeholder-indigo-400 focus:border-indigo-400 focus:ring-indigo-400 text-sm py-3">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="building_number" id="building_number" required value="{{ old('building_number', $property->building_number) }}" placeholder="e.g. House No. 42B, Flat No. 3" class="block w-full rounded-xl bg-indigo-900/40 border-indigo-800 text-white placeholder-indigo-400 focus:border-indigo-400 focus:ring-indigo-400 text-sm py-3">
                </div>
        ...
        <div class="bg-white border border-slate-200/80 rounded-2xl shadow-sm p-6 lg:p-8 space-y-6">
            ...
            @if($property->images->count() > 0)
                <div>
                    <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">Current Flat Photos</label>
                    <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-4">
                        @foreach($property->images as $img)
                            <div class="relative rounded-xl overflow-hidden aspect-video border border-slate-100 group shadow-sm bg-slate-50">
                                <img src="{{ asset('storage/' . $img->file_path) }}" alt="Flat image" class="object-cover w-full h-full">
                                @if($img->is_featured)
                                    <span class="absolute top-2 left-2 bg-indigo-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">Featured</span>
                                @endif
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Media Upload -->
                <div>
                    <label for="images" class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Upload More Flat Photos</label>
                ...
                @if(auth()->user()->hasRole('admin'))
                    <div>
                        ...
                        <select name="verification_status" id="verification_status" class="block w-full rounded-xl border-slate-200/85 focus:border-indigo-500 focus:ring-indigo-500 text-sm py-3">
                            <option value="pending" {{ old('verification_status', $property->verification_status) === 'pending' ? 'selected' : '' }}>Pending Approval</option>
                            <option value="verified" {{ old('verification_status', $property->verification_status) === 'verified' ? 'selected' : '' }}>Verified (Safe for Public Listing)</option>
                            <option value="rejected" {{ old('verification_status', $property->verification_status) === 'rejected' ? 'selected' : '' }}>Rejected</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="publication_status" id="publication_status" class="block w-full rounded-xl border-slate-200/85 focus:border-indigo-500 focus:ring-indigo-500 text-sm py-3">
                            <option value="draft" {{ old('publication_status', $property->publication_status) === 'draft' ? 'selected' : '' }}>Draft (Offline)</option>
                            <option value="published" {{ old('publication_status', $property->publication_status) === 'published' ? 'selected' : '' }}>Published (Live on Site)</option>
                            <option value="archived" {{ old('publication_status', $property->publication_status) === 'archived' ? 'selected' : '' }}>Archived</option>
                        </select>
                    </div>
                @endif
            </div>
